...
import { operationsTable, operationTasksTable } from "@workspace/db/schema";
...
router.post("/:id/tasks", async (req, res) => {
  try {
    const [row] = await db.insert(operationTasksTable).values({ ...req.body, operationId: parseInt(req.params.id) }).returning();
...
router.patch("/:id/tasks/:taskId", async (req, res) => {
  try {
    ...
    const [row] = await db.update(operationTasksTable).set(body).where(eq(operationTasksTable.id, parseInt(req.params.taskId))).returning();
    await updateCompletionRate(parseInt(req.params.id));
    res.json(row);
...
async function updateCompletionRate(operationId: number) {
  ...
  await db.update(operationsTable).set({ completionRate: rate }).where(eq(operationsTable.id, operationId));
}
