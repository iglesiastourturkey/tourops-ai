...
import { operationReceiptsTable } from "@workspace/db/schema";
 ...
 * Requires a valid Clerk session (this is a single-tenant app; all
 * authenticated users are agency staff of the same organisation).
 ...
 *   1. requireAuth — user must have a valid Clerk session. This is a
 *      single-tenant app; all authenticated users are the same agency's staff.
 ...
 *      the agency's receipt workflow, preventing serving of arbitrary paths
 *      (path guessing, path traversal, or reuse of paths from other contexts).
 */
router.get("/storage/objects/*path", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // in the DB. Prevents any authenticated user from fetching arbitrary GCS
    // objects by guessing or constructing paths not generated by this workflow.
