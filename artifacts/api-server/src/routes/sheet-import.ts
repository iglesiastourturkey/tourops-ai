...
//
// Faz 5.3: hardening pass, from the GEMI ingestion architecture review's two
// confirmed, live findings:
//   1. /approve always inserted a brand-new customer, even when the sheet
//      row's phone/email already matched an existing one. It now matches by
//      email then phone first (same order as POST /reservations/:id/create-draft),
//      and only creates a customer when nothing matches.
//   2. A GEMI row that gets edited after being approved comes back through
...
// for "is this row currently pending/approved/rejected" - approvedAt/
// approvedBy mean "the last time this row was approved", not "is it approved
// right now". Every place that used to branch on approvedAt/rejectedAt being
// null now branches on status instead (see the GET/PATCH/POST handlers
// below, and sheet-import-review.tsx on the frontend).
    ...
    const [inserted] = await db
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          rejectedAt: null,
          rejectedBy: null,
          // Faz 5.3: approvedAt/approvedBy are deliberately left untouched
          // here. They used to be nulled on every edit, which made a
          // re-approval indistinguishable from a first approval and is what
          // let /approve silently create a duplicate operation (see the
          // header comment). status already carries "is this row currently
          // pending" - approvedAt/approvedBy now carry "when/by whom was it
          // last approved", which /approve reads to detect whether the
          // operation it created was touched in TourPilot since then. The
          // next successful /approve overwrites both anyway.
        },
      })
 ...
 * Defaults to "pending".
 *
 * Faz 5.3: filters on the status column, not on approvedAt/rejectedAt being
 * null - now that a re-approvable row can carry a non-null approvedAt while
 * status is back to "pending" (see the header comment), the old
 * approvedAt-nullness check would have misfiled it under "approved".
 */
 ...
 *
 * Faz 5.3: two behavior changes, both from confirmed findings in the GEMI
 * ingestion architecture review (see the header comment for the full
 * rationale):
 *   - Customer: matches an existing customer by email then phone (same
 *     order as create-draft) before creating a new one. A row that already
 *     has a linked customer from an earlier approval keeps that link rather
 *     than re-matching.
...
router.post("/:id/approve", async (req, res) => {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      // a new one. A row with an existing link keeps it: the sheet is not
      // the place to decide a booking now belongs to a different customer.
      let customerId: number | null = null;
      if (previouslyMatchedCustomerId) {
        const [linked] = await tx
          .select({ id: customersTable.id })
          .from(customersTable)
          .where(eq(customersTable.id, previouslyMatchedCustomerId))
          .limit(1);
        if (linked) customerId = linked.id;
      }
      if (!customerId && fields.customerName) {
        let matched: { id: number } | undefined;
        if (fields.customerEmail) {
          [matched] = await tx
            .select({ id: customersTable.id })
            .from(customersTable)
            .where(sql`lower(trim(${customersTable.email})) = lower(trim(${fields.customerEmail}))`)
            .limit(1);
        }
        if (!matched && fields.customerPhone) {
          [matched] = await tx
            .select({ id: customersTable.id })
            .from(customersTable)
            .where(sql`lower(trim(${customersTable.phone})) = lower(trim(${fields.customerPhone}))`)
            .limit(1);
        }
        if (matched) {
      ...
      if (previouslyMatchedOperationId) {
        [operation] = await tx
          ...
          .set(operationValues)
          .where(eq(operationsTable.id, previouslyMatchedOperationId))
          .returning();
      }
      if (!operation) {
        [operation] = await tx
