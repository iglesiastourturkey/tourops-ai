...
router.post("/exchange-rates", requirePermission("settings", "manage"), async (req, res) => {
  try {
...
router.patch("/exchange-rates/:id", requirePermission("settings", "manage"), async (req, res) => {
  try {
...
router.delete("/exchange-rates/:id", requirePermission("settings", "manage"), async (req, res) => {
  try {
...
// --- Agency settings ---
router.get("/agency-settings", requirePermission("settings", "view"), async (req, res) => {
  try {
...
router.patch("/agency-settings", requirePermission("settings", "manage"), async (req, res) => {
  try {
...
// --- Email templates ---
router.get("/email-templates", requirePermission("settings", "view"), async (req, res) => {
  try {
    res.json(await db.select().from(emailTemplatesTable));
  } catch { res.status(500).json({ error: "Failed to list email templates" }); }
});

router.post("/email-templates", requirePermission("settings", "manage"), async (req, res) => {
  try {
...
router.get("/email-templates/:id", requirePermission("settings", "view"), async (req, res) => {
  try {
...
router.patch("/email-templates/:id", requirePermission("settings", "manage"), async (req, res) => {
  try {
...
router.delete("/email-templates/:id", requirePermission("settings", "manage"), async (req, res) => {
  try {
