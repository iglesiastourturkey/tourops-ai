...
export default function HistoricalRemediationPage() {
  ...
  return <AppShell title="Tarihsel İyileştirme"><main className="mx-auto max-w-7xl space-y-5 p-4 md:p-6">
    ...
    {query.data && <div className="space-y-3"><p className="text-sm text-muted-foreground">{query.data.total} kayıt</p>{query.data.rows.map(row => <Link key={row.id} href={`/historical-remediation/${row.id}`}><Card className="mb-3 cursor-pointer p-4 transition-colors hover:bg-muted/40"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-medium">{row.customerName || 'İsimsiz kayıt'}</p><p className="text-xs text-muted-foreground">{row.sourceKey}</p><p className="mt-1 text-sm">{row.operationDate} · {row.worksheetName} · satır {row.sourceRow}</p></div><Badge variant={row.derivedState === 'READY_FOR_REVIEW' ? 'default' : 'secondary'}>{row.derivedState}</Badge></div><div className="mt-3 flex flex-wrap gap-2">{row.missingFields.map(field => <Badge variant="outline" key={field}>{fieldLabels[field] ?? field}</Badge>)}{!row.hasSourceEvidence && <Badge variant="destructive">Kaynak kanıtı yok</Badge>}</div></Card></Link>)}</div>}
  </main></AppShell>;
}
