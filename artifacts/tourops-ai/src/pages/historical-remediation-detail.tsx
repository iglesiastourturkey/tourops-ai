...
import { HistoricalRemediationPanel } from '@/components/historical-remediation/remediation-panel';
import { HistoricalApprovalHandoffPanel } from '@/components/historical-remediation/approval-handoff-panel';
...
export default function HistoricalRemediationDetailPage() {
  ...
  return <AppShell title="Kaynak Kanıtı"><main className="mx-auto max-w-6xl space-y-5 p-4 md:p-6">
    ...
    {row && <><div><div className="flex flex-wrap items-center gap-2"><h1 className="text-2xl font-bold text-[#1e3a5f]">{row.customerName || 'İsimsiz kayıt'}</h1><Badge variant={row.derivedState === 'READY_FOR_REVIEW' ? 'default' : 'secondary'}>{row.derivedState === 'READY_FOR_REVIEW' ? 'İncelemeye Hazır' : `Çözülmedi · ${row.warnings.length} uyarı`}</Badge></div><p className="text-xs text-muted-foreground">{row.sourceKey}</p></div>
      ...
      <HistoricalRemediationPanel row={row} />
      <HistoricalApprovalHandoffPanel row={row} />
