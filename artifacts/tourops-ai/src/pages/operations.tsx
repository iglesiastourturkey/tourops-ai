          ...
          <TableBody>
            {isLoading ? Array.from({ length: 5 }).map((_, i) => (
              ...
              <TableRow key={op.id} data-testid={`row-operation-${op.id}`}>
                ...
                <TableCell><span className={`text-xs px-2 py-0.5 rounded-full font-medium ${OPERATION_STATUS_COLORS[op.status] ?? 'bg-gray-100 text-gray-600'}`}>{OPERATION_STATUS_LABELS[op.status] ?? op.status}</span></TableCell>
                <TableCell>
                ...
                <TableCell>
                  <Link href={`/operations/${op.id}`}><Button size="icon" variant="ghost" className="h-7 w-7" data-testid={`button-view-operation-${op.id}`}><ExternalLink className="w-3.5 h-3.5" /></Button></Link>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
